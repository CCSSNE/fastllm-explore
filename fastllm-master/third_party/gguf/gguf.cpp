...
#if defined(_MSC_VER) || (defined(__GNUC__) && __GNUC__ < 8 && !defined(__clang__))
...
namespace fastllm {
    GGUFBuffer::GGUFBuffer (const std::string &fileName) {
        ...
        this->f = FastllmFileOpen(fileName, "rb");
        if (this->f == nullptr) {
            ErrorInFastLLM("GGUFBuffer: can't open file " + fileName + "\n");
        }
    }
    ...
    T GGUFBuffer::Read() {
        ...
        size_t got = fread(&v, 1, sizeof(T), f);
        if (got != sizeof(T)) {
            ErrorInFastLLM("GGUFBuffer.Read error at " + std::to_string(pos) +
    ...
    void WeightImportGGUFTensor(Data* weight, ggml_tensor *tensor, std::string &fileName, uint64_t offset, 
        ...
        if (replaceType == GGUFWeightReplaceRule::GGUFWeightReplaceDirect) {
            ...
            FILE *fi = FastllmFileOpen(fileName, "rb");
...
*/
            ...
            if (tensor->type == GGML_TYPE_F32) {
                memcpy(floatData.data(), oriData.data(), len);
            } else {
                auto toFloat = ggml_type_to_float(tensor->type);
                AssertInFastLLM(toFloat != nullptr, "WeightImportGGUFTensor: weight " + tensor->name + "(type " +
                    ggml_type_name(tensor->type) + ") can't convert to fp32.");
        ...
        for (int i = 0; i < metaDataCount; i++) {
            ...
            if (type == GGUF_TYPE_STRING) {
                ...
                if (type == GGUF_TYPE_STRING) {
                    ...
                    paramsConfig[key] = value;
                } else if (type == GGUF_TYPE_UINT8 || type == GGUF_TYPE_INT8) {
                    for (int i = 0; i < n; i++) {
                        ggufBuffer.Read <int8_t> ();
                    }
                } else if (type == GGUF_TYPE_UINT16 || type == GGUF_TYPE_INT16) {
                    for (int i = 0; i < n; i++) {
                        ggufBuffer.Read <int16_t> ();
                    }
                } else if (type == GGUF_TYPE_UINT32) {
                    for (int i = 0; i < n; i++) {
                ...
                } else if (type == GGUF_TYPE_UINT64 || type == GGUF_TYPE_INT64 || type == GGUF_TYPE_FLOAT64) {
                    for (int i = 0; i < n; i++) {

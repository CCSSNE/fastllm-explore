...
namespace fastllm {
    ...
    struct SafeTensorItem {
        ...
        void CreateBufferWithScale(DataType dstType, SafeTensorItem &scale) {
            ...
            if (dstType == DataType::FP8_E4M3 || dstType == DataType::NVFP4) {
                ...
                FILE *fi = FastllmFileOpen(this->fileName, "rb");
        ...
        void CreateBufferWithAWQ(DataType dstType, SafeTensorItem &scale, SafeTensorItem &qzero) {
            ...
            ClearBuffer();
            FILE *fweight = FastllmFileOpen(this->fileName, "rb");
            FILE *fqzero  = FastllmFileOpen(qzero.fileName, "rb");
#if defined(_WIN32) || defined(_WIN64)
    ...
    struct SafeTensors {
        ...
        SafeTensors (const std::set <std::string> &fileNames) {
            ...
            for (auto &fileName : fileNames) {
                FILE *f = FastllmFileOpen(fileName, "rb");
    ...
    static bool IsDiskMoeWeight(basellm *model, const std::string &weightName, bool useDiskMoe) {
        if (weightName.find(".ffn.shared_experts.") != std::string::npos) {
            return false;
        }
...
// printf("eos_token = %s\n", eos_token.c_str());
            ...
            if (arch == "deepseek4" || arch == "deepseek_v4") {
                if (!params[arch + ".attention.q_lora_rank"].is_null()) {
                    model->weight.AddDict("q_lora_rank", std::to_string(params[arch + ".attention.q_lora_rank"].int_value()));
                }
                if (!params[arch + ".attention.output_lora_rank"].is_null()) {
                    model->weight.AddDict("o_lora_rank", std::to_string(params[arch + ".attention.output_lora_rank"].int_value()));
                }
                if (!params[arch + ".attention.output_group_count"].is_null()) {
                    model->weight.AddDict("o_groups", std::to_string(params[arch + ".attention.output_group_count"].int_value()));
                }
                if (!params[arch + ".attention.key_length"].is_null()) {
                    model->weight.AddDict("head_dim", std::to_string(params[arch + ".attention.key_length"].int_value()));
                }
                if (!params[arch + ".rope.dimension_count"].is_null()) {
                    model->rotary_dim = params[arch + ".rope.dimension_count"].int_value();
                    model->weight.AddDict("qk_rope_head_dim", std::to_string(model->rotary_dim));
                }
                if (!params[arch + ".attention.sliding_window"].is_null()) {
                    model->weight.AddDict("sliding_window", std::to_string(params[arch + ".attention.sliding_window"].int_value()));
                }
                if (!params[arch + ".attention.indexer.head_count"].is_null()) {
                    model->weight.AddDict("index_n_heads", std::to_string(params[arch + ".attention.indexer.head_count"].int_value()));
                }
                if (!params[arch + ".attention.indexer.key_length"].is_null()) {
                    model->weight.AddDict("index_head_dim", std::to_string(params[arch + ".attention.indexer.key_length"].int_value()));
                }
                if (!params[arch + ".attention.indexer.top_k"].is_null()) {
                    model->weight.AddDict("index_topk", std::to_string(params[arch + ".attention.indexer.top_k"].int_value()));
                }
                ...
                if (!params[arch + ".attention.compress_rope_freq_base"].is_null()) {
                    model->weight.AddDict("compress_rope_theta", std::to_string(params[arch + ".attention.compress_rope_freq_base"].number_value()));
                }
                if (!params[arch + ".expert_feed_forward_length"].is_null()) {
                    model->weight.AddDict("moe_intermediate_size", std::to_string(params[arch + ".expert_feed_forward_length"].int_value()));
                }
                if (!params[arch + ".expert_count"].is_null()) {
                    model->weight.AddDict("n_routed_experts", std::to_string(params[arch + ".expert_count"].int_value()));
                }
                if (!params[arch + ".expert_used_count"].is_null()) {
                    model->weight.AddDict("num_experts_per_tok", std::to_string(params[arch + ".expert_used_count"].int_value()));
                }
                if (!params[arch + ".expert_shared_count"].is_null()) {
                    model->weight.AddDict("n_shared_experts", std::to_string(params[arch + ".expert_shared_count"].int_value()));
                }
                if (!params[arch + ".expert_weights_scale"].is_null()) {
                    model->weight.AddDict("routed_scaling_factor", std::to_string(params[arch + ".expert_weights_scale"].number_value()));
                }
                if (!params[arch + ".expert_weights_norm"].is_null()) {
                    model->weight.AddDict("norm_topk_prob", params[arch + ".expert_weights_norm"].bool_value() ? "true" : "false");
                }
                if (!params[arch + ".swiglu_clamp_exp"].is_null() && params[arch + ".swiglu_clamp_exp"].is_array() &&
                    !params[arch + ".swiglu_clamp_exp"].array_items().empty()) {
                    model->weight.AddDict("swiglu_limit", std::to_string(params[arch + ".swiglu_clamp_exp"].array_items()[0].number_value()));
                }
                if (!params[arch + ".hash_layer_count"].is_null()) {
                    model->weight.AddDict("num_hash_layers", std::to_string(params[arch + ".hash_layer_count"].int_value()));
                }
                if (!params[arch + ".nextn_predict_layers"].is_null()) {
                    model->weight.AddDict("num_nextn_predict_layers", std::to_string(params[arch + ".nextn_predict_layers"].int_value()));
                }
                if (!params[arch + ".hyper_connection.count"].is_null()) {
                    model->weight.AddDict("hc_mult", std::to_string(params[arch + ".hyper_connection.count"].int_value()));
                }
                if (!params[arch + ".hyper_connection.sinkhorn_iterations"].is_null()) {
                    model->weight.AddDict("hc_sinkhorn_iters", std::to_string(params[arch + ".hyper_connection.sinkhorn_iterations"].int_value()));
                }
                if (!params[arch + ".hyper_connection.epsilon"].is_null()) {
                    model->weight.AddDict("hc_eps", std::to_string(params[arch + ".hyper_connection.epsilon"].number_value()));
                }
                if (!params[arch + ".rope.freq_base"].is_null()) {
                    model->weight.AddDict("rope_theta", std::to_string(params[arch + ".rope.freq_base"].number_value()));
                }
                ...
                if (!params[arch + ".rope.scaling.factor"].is_null()) {
                    model->weight.AddDict("rope_scaling.factor", std::to_string(params[arch + ".rope.scaling.factor"].number_value()));
                }
                if (!params[arch + ".rope.scaling.original_context_length"].is_null()) {
                    model->weight.AddDict("rope_scaling.original_max_position_embeddings", std::to_string(params[arch + ".rope.scaling.original_context_length"].number_value()));
                }
                if (!params[arch + ".rope.scaling.yarn_beta_fast"].is_null()) {
                    model->weight.AddDict("rope_scaling.beta_fast", std::to_string(params[arch + ".rope.scaling.yarn_beta_fast"].int_value()));
                }
                if (!params[arch + ".rope.scaling.yarn_beta_slow"].is_null()) {
                    model->weight.AddDict("rope_scaling.beta_slow", std::to_string(params[arch + ".rope.scaling.yarn_beta_slow"].int_value()));
                }
            }
            ...
            int idx = 0;
            std::map<std::string, int> ggufSpecialTokens;
            std::vector<json11::Json> tokenTypes = params["tokenizer.ggml.token_type"].array_items();
            for (auto &it : params["tokenizer.ggml.tokens"].array_items()) {
                std::string token = it.string_value();
                if (idx < 10) {
                    // printf("%s: %d\n", it.string_value().c_str(), idx);
                }
                model->weight.AddTokenizerWord(token, idx, 1.0f);
                bool isSpecialToken = token.find("<｜") != std::string::npos ||
                                      token.find("｜>") != std::string::npos ||
                                      token == "<think>" || token == "</think>" ||
                                      token.find("｜DSML｜") != std::string::npos;
                if (idx < (int)tokenTypes.size()) {
                    int tokenType = tokenTypes[idx].int_value();
                    isSpecialToken = isSpecialToken || (tokenType != 1 && tokenType != 6);
                }
                if (isSpecialToken) {
                    ggufSpecialTokens[token] = idx;
                }
                idx++;
            }
            ...
            model->weight.tokenizer.byteAsChar = true;
            model->weight.AddDict("tokenizer_byte_as_char", "True");
            if (!ggufSpecialTokens.empty()) {
                model->weight.tokenizer.SetSpecialTokens(ggufSpecialTokens);
                model->weight.AddDict("tokenizer_has_special_tokens", "1");
                printf("Load gguf special tokens = %d\n", (int)ggufSpecialTokens.size());
            }

...
// printf("into fastllm gemm %s %s %s\n", GetDataTypeName(AType).c_str(), GetDataTypeName(BType).c_str(), GetDataTypeName(CType).c_str());
 ...
 // std::vector <std::pair <std::string, float> > record;
        ...
        float swigluLimit = floatParams.find("swigluLimit") != floatParams.end() ? floatParams.find("swigluLimit")->second : 0.0f;
            ...
            for (int o = 0; o < outer; o++) {
                ...
                for (int st = 0; st < v.size(); st++) {
                    ...
                    for (int l = st; l <= end; l++) {
                        int idx = v[l].first;
                        int mid = weights[idx * 2]->dims[0] / 2;
                        float routeValue = v[l].second;
                        float *middle = middles[l].data();
                        if (!useGeglu && swigluLimit > 0.0f) {
                            for (int q = 0; q < mid; q++) {
                                float gate = std::min(middle[q], swigluLimit);
                                float up = std::max(-swigluLimit, std::min(middle[mid + q], swigluLimit));
                                middle[q] = (gate / (1.0f + expf(-gate))) * up;
                            }
                        }
                        if (routeValue != 1.0f) {
                            for (int q = 0; q < mid; q++) {
                        ...
                        auto &uinputDown = q8kInputsDown[l];
                        fastllm::MultiThreadFloat32ToQ8KOp(middle, (uint8_t*)uinputDown.data(), mid, (ggml_type)weights[idx * 2 + 1]->ggmlType).Run();
                    }
/*
                    for (int l = st; l <= end; l++) {
                        int idx = v[l].first;
  ...
  // }
    ...
    void CpuEmbedding::Run(const std::string &opType, const fastllm::DataDict &datas,
        ...
        if (GetLowMemMode() && !weight.fileName.empty()) {
            FILE *fi = FastllmFileOpen(weight.fileName, "rb");

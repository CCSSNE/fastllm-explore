...
namespace fastllm {
    ...
    namespace {
        ...
        static bool DeepSeekV4ProfileEnabled() {
            return EnvFlagEnabled("FASTLLM_PROFILE_DEEPSEEKV4") ||
                   EnvFlagEnabled("FASTLLM_DSV4_PROFILE");
        }

        static void DeepSeekV4ProfileLog(const char *stage, int layer,
                                         int startPos, int seqlen,
                                         double startMs) {
            if (!DeepSeekV4ProfileEnabled()) {
                return;
            }
            double elapsed = (NowMs() - startMs) * 0.001;
            if (layer >= 0) {
                printf("[fastllm-dsv4-profile] layer=%d stage=%s start=%d seq=%d elapsed=%.3fs\n",
                       layer, stage, startPos, seqlen, elapsed);
            } else {
                printf("[fastllm-dsv4-profile] stage=%s start=%d seq=%d elapsed=%.3fs\n",
                       stage, startPos, seqlen, elapsed);
            }
            fflush(stdout);
        }
        ...
        static std::vector<float> ReadFloatData(const Data &input) {
            ...
            if (input.dataType == DataType::DATA_GGUF_FORMAT && input.ggmlType == GGML_TYPE_I32) {
                Data tmp;
                tmp.CopyFrom(input);
                tmp.ToDevice(DataDevice::CPU);
                uint64_t cnt = tmp.Count(0) / sizeof(int32_t);
                std::vector<float> ret(cnt);
                int32_t *p = (int32_t*)tmp.cpuData;
                for (uint64_t i = 0; i < cnt; i++) {
                    ret[i] = (float)p[i];
        ...
        struct WoAReferenceGGUFOp : MultiThreadBaseOp {
            const std::vector<float> *ov;
            const uint8_t *weightData;
            ggml_to_float_t toFloat;
            size_t rowBytes;
            float *y;
            int st, end;
            int bsz, seqlen, heads, headDim, groups, oRank, headsPerGroup, groupDim;

            WoAReferenceGGUFOp(const std::vector<float> *ov, const uint8_t *weightData,
                               ggml_to_float_t toFloat, size_t rowBytes, float *y,
                               int st, int end, int bsz, int seqlen, int heads, int headDim,
                               int groups, int oRank)
                : ov(ov), weightData(weightData), toFloat(toFloat), rowBytes(rowBytes),
                  y(y), st(st), end(end), bsz(bsz), seqlen(seqlen),
                  heads(heads), headDim(headDim), groups(groups), oRank(oRank) {
                headsPerGroup = heads / groups;
                groupDim = headsPerGroup * headDim;
            }
            ...
            void Run() override {
                ...
                for (int idx = st; idx < end; idx++) {
                    ...
                    int s = tmp % seqlen;
                    int b = tmp / seqlen;
                    int rowIndex = g * oRank + r;
                    toFloat(weightData + (uint64_t)rowIndex * rowBytes, row.data(), groupDim);
                    double v = 0.0;
                    int d = 0;
                    for (int hh = 0; hh < headsPerGroup; hh++) {
                        const float *src = ovData + (((uint64_t)b * seqlen + s) * heads +
        ...
        static bool WoAReferenceGGUF(Data &o, Data &woA, int groups, int oRank, Data &output) {
            if (woA.dataType != DataType::DATA_GGUF_FORMAT || woA.ggmlType < 0 ||
                o.dims.size() != 4 || woA.dims.size() != 2) {
                return false;
            }
            int bsz = o.dims[0], seqlen = o.dims[1], heads = o.dims[2], headDim = o.dims[3];
            if (groups <= 0 || oRank <= 0 || heads % groups != 0) {
                return false;
            }
            int headsPerGroup = heads / groups;
            int groupDim = headsPerGroup * headDim;
            int rows = groups * oRank;
            if (woA.dims[0] != rows || woA.dims[1] != groupDim) {
                return false;
            }
            auto toFloat = ggml_type_to_float((ggml_type)woA.ggmlType);
            if (toFloat == nullptr) {
                return false;
            }
            woA.ToDevice(DataDevice::CPU);
            if (woA.cpuData == nullptr) {
                return false;
            }

            auto ov = ReadFloatData(o);
            size_t rowBytes = ggml_row_size((ggml_type)woA.ggmlType, groupDim);
            std::vector<float> y((uint64_t)bsz * seqlen * rows, 0.0f);
            int total = bsz * seqlen * rows;
            auto *pool = GetAlivePool();
            int threadNum = std::min((int)pool->threads.size(), total);
            if (threadNum <= 1 || total < 1024) {
                WoAReferenceGGUFOp(&ov, woA.cpuData, toFloat, rowBytes, y.data(),
                                   0, total, bsz, seqlen, heads, headDim, groups, oRank).Run();
            } else {
                std::vector<WoAReferenceGGUFOp*> ops;
                int per = total / threadNum;
                int cur = 0;
                for (int i = 0; i < threadNum; i++) {
                    int end = (i == threadNum - 1) ? total : cur + per;
                    ops.push_back(new WoAReferenceGGUFOp(&ov, woA.cpuData, toFloat, rowBytes, y.data(),
                                                        cur, end, bsz, seqlen, heads, headDim, groups, oRank));
                    cur = end;
                }
                for (int i = 0; i < (int)ops.size(); i++) {
                    pool->PushOp(i, ops[i]);
            ...
            WriteFloatData(y, {bsz, seqlen, rows}, output, DataType::BFLOAT16);
            return true;
        }

        static void WoAReference(Data &o, Data &woA, int groups, int oRank, Data &output) {
            if (WoAReferenceGGUF(o, woA, groups, oRank, output)) {
                return;
            }
            auto ov = ReadFloatData(o);
    ...
    int DeepSeekV4Model::Forward(const fastllm::Data &inputIds, const fastllm::Data &attentionMask,
        ...
        DeepSeekV4ProfileLog("forward-entry", -1, -1,
                             inputIds.dims.size() >= 2 ? inputIds.dims[1] : -1,
                             NowMs());
    ...
    std::vector <int> DeepSeekV4Model::ForwardBatch(int batch,
        ...
        bool profileDsv4 = DeepSeekV4ProfileEnabled();
        double forwardStartMs = NowMs();
        if (profileDsv4) {
            auto *executor = GetProfilerExecutor();
            if (executor != nullptr) {
        ...
        DeepSeekV4ProfileLog("embedding-before", -1, startPos,
                             inputIds.dims.size() >= 2 ? inputIds.dims[1] : -1,
                             forwardStartMs);
        Embedding(inputIds, weight["embed.weight"], hiddenStates);
        DeepSeekV4ProfileLog("embedding-after", -1, startPos,
                             inputIds.dims.size() >= 2 ? inputIds.dims[1] : -1,
                             forwardStartMs);

        int bsz = inputIds.dims[0];
        int seqlen = inputIds.dims[1];
        DeepSeekV4ProfileLog("forward-start", -1, startPos, seqlen, forwardStartMs);
        int dim = embed_dim;
        {
        ...
        for (int layer = 0; layer < block_cnt; layer++) {
            ...
            double layerStartMs = NowMs();
            DeepSeekV4ProfileLog("layer-start", layer, startPos, seqlen, layerStartMs);

            DeepSeekV4ProfileLog("hc-attn-before", layer, startPos, seqlen, layerStartMs);
            ...
            DeepSeekV4ProfileLog("hc-attn-after", layer, startPos, seqlen, layerStartMs);
            Data attnInput;
            DeepSeekV4ProfileLog("attn-norm-before", layer, startPos, seqlen, layerStartMs);
            RMSNormReference(attnMix.y, weight[pre + ".attn_norm.weight"], rms_norm_eps, attnInput, DataType::BFLOAT16);
            DeepSeekV4ProfileLog("attn-norm-after", layer, startPos, seqlen, layerStartMs);

            Data qr, qNorm, q;
            DeepSeekV4ProfileLog("wq-a-before", layer, startPos, seqlen, layerStartMs);
            Linear(attnInput, weight[pre + ".attn.wq_a.weight"], Data(), qr);
            DeepSeekV4ProfileLog("wq-a-after", layer, startPos, seqlen, layerStartMs);
            DeepSeekV4ProfileLog("q-norm-before", layer, startPos, seqlen, layerStartMs);
            RMSNormReference(qr, weight[pre + ".attn.q_norm.weight"], rms_norm_eps, qNorm, DataType::BFLOAT16);
            DeepSeekV4ProfileLog("q-norm-after", layer, startPos, seqlen, layerStartMs);
            DeepSeekV4ProfileLog("wq-b-before", layer, startPos, seqlen, layerStartMs);
            Linear(qNorm, weight[pre + ".attn.wq_b.weight"], Data(), q);
            DeepSeekV4ProfileLog("wq-b-after", layer, startPos, seqlen, layerStartMs);
            q.Reshape({bsz, seqlen, num_attention_heads, head_dim_full});
            DeepSeekV4ProfileLog("scale-q-before", layer, startPos, seqlen, layerStartMs);
            ScaleQRotary(q, rms_norm_eps, qk_rope_head_dim, layerRopeBase, startPos,
                         ...
                         rope_scaling_beta_slow);
            DeepSeekV4ProfileLog("scale-q-after", layer, startPos, seqlen, layerStartMs);

            Data kv;
            DeepSeekV4ProfileLog("wkv-before", layer, startPos, seqlen, layerStartMs);
            Linear(attnInput, weight[pre + ".attn.wkv.weight"], Data(), kv);
            DeepSeekV4ProfileLog("wkv-after", layer, startPos, seqlen, layerStartMs);
            DeepSeekV4ProfileLog("kv-norm-before", layer, startPos, seqlen, layerStartMs);
            RMSNormReference(kv, weight[pre + ".attn.kv_norm.weight"], rms_norm_eps, kv, DataType::BFLOAT16);
            DeepSeekV4ProfileLog("kv-norm-after", layer, startPos, seqlen, layerStartMs);
            kv.Reshape({bsz, seqlen, 1, head_dim_full});
            DeepSeekV4ProfileLog("rotary-kv-before", layer, startPos, seqlen, layerStartMs);
            RotaryQuant(kv, qk_rope_head_dim, layerRopeBase, startPos,
                        layerOriginalSeqLen, rope_factor, rope_scaling_beta_fast,
                        rope_scaling_beta_slow, head_dim_full - qk_rope_head_dim, 64);
            DeepSeekV4ProfileLog("rotary-kv-after", layer, startPos, seqlen, layerStartMs);
            ...
            if (decodeCache != nullptr) {
                DeepSeekV4ProfileLog("decode-cache-before", layer, startPos, seqlen, layerStartMs);
                if (startPos == 0) {
                ...
                DeepSeekV4ProfileLog("decode-cache-after", layer, startPos, seqlen, layerStartMs);
            }
            ...
            if (compressRatio > 0) {
                if (decodeCache != nullptr) {
                    ...
                    DeepSeekV4ProfileLog("compressor-raw-before", layer, startPos, seqlen, layerStartMs);
                    ComputeCompressorRaw(weight, pre + ".attn.compressor", attnInput, compressorKV, compressorScore);
                    DeepSeekV4ProfileLog("compressor-raw-after", layer, startPos, seqlen, layerStartMs);
                    if (startPos == 0) {
                    ...
                    Data compressedKV;
                    DeepSeekV4ProfileLog("compress-kv-before", layer, startPos, seqlen, layerStartMs);
                    ...
                    DeepSeekV4ProfileLog("compress-kv-after", layer, startPos, seqlen, layerStartMs);
                }
            }
            ...
            if (decodeCache != nullptr && startPos > 0 && seqlen == 1) {
                DeepSeekV4ProfileLog("sparse-attn-decode-before", layer, startPos, seqlen, layerStartMs);
                SparseAttentionDecodeCachedReference(q, decodeCache->windowKV, &decodeCache->windowKVData,
                                                     *decodeCompressedKVForAttention, weight[pre + ".attn.attn_sink"],
                                                     ...
                                                     rope_scaling_beta_fast, rope_scaling_beta_slow);
                DeepSeekV4ProfileLog("sparse-attn-decode-after", layer, startPos, seqlen, layerStartMs);
            } else {
                DeepSeekV4ProfileLog("sparse-attn-before", layer, startPos, seqlen, layerStartMs);
                ...
                DeepSeekV4ProfileLog("sparse-attn-after", layer, startPos, seqlen, layerStartMs);
            }
            if (releaseCpuCompressedAfterSparse) {
                ResetData(decodeCache->compressedKV);
            }
            DeepSeekV4ProfileLog("wo-a-before", layer, startPos, seqlen, layerStartMs);
            WoA(attnOut4, weight[pre + ".attn.wo_a.weight"], o_groups, o_lora_rank, woAOut);
            DeepSeekV4ProfileLog("wo-a-after", layer, startPos, seqlen, layerStartMs);
            DeepSeekV4ProfileLog("wo-b-before", layer, startPos, seqlen, layerStartMs);
            Linear(woAOut, weight[pre + ".attn.wo_b.weight"], Data(), attnOut);
            DeepSeekV4ProfileLog("wo-b-after", layer, startPos, seqlen, layerStartMs);
            DeepSeekV4ProfileLog("hc-post-attn-before", layer, startPos, seqlen, layerStartMs);
            HcPostReference(attnOut, hiddenStates, attnMix, hiddenStates);
            DeepSeekV4ProfileLog("hc-post-attn-after", layer, startPos, seqlen, layerStartMs);
            DeepSeekV4ProfileLog("attention-done", layer, startPos, seqlen, layerStartMs);

            DeepSeekV4ProfileLog("hc-ffn-before", layer, startPos, seqlen, layerStartMs);
            HcMix ffnMix = HcPreReference(hiddenStates, weight[pre + ".hc_ffn_fn"],
                                          weight[pre + ".hc_ffn_scale"], weight[pre + ".hc_ffn_base"],
                                          hc_mult, hc_sinkhorn_iters, hc_eps, rms_norm_eps);
            DeepSeekV4ProfileLog("hc-ffn-after", layer, startPos, seqlen, layerStartMs);
            Data ffnInput, ffnOut;
            DeepSeekV4ProfileLog("ffn-norm-before", layer, startPos, seqlen, layerStartMs);
            RMSNormReference(ffnMix.y, weight[pre + ".ffn_norm.weight"], rms_norm_eps, ffnInput, DataType::BFLOAT16);
            DeepSeekV4ProfileLog("ffn-norm-after", layer, startPos, seqlen, layerStartMs);
            std::vector<int> ffnDims = ffnInput.dims;
            ffnInput.Reshape({bsz * seqlen, dim});
            Data expertIndex, expertScore;
            DeepSeekV4ProfileLog("moe-route-before", layer, startPos, seqlen, layerStartMs);
            BuildMoERoutingData(weight, pre + ".ffn", ffnInput, tokenIds, num_experts,
                                num_experts_per_tok, scoring_func, routed_scaling_factor,
                                expertIndex, expertScore);
            DeepSeekV4ProfileLog("moe-route-after", layer, startPos, seqlen, layerStartMs);
            {
                // MOE
                Data w1, w2, w3, tempInput, tempOutput, moeInputTemp, moeOutputTemp, sharedExpertOut;
                Data ww1, ww3;
                DeepSeekV4ProfileLog("shared-expert-before", layer, startPos, seqlen, layerStartMs);
                if (cudaSe &&
                    ...
                    weights[layer][0] = weights[layer][1] = nullptr;
                           ...
                           weight.weight.find(pre + ".ffn.shared_experts.w1.weight") != weight.weight.end() &&
                           weight.weight.find(pre + ".ffn.shared_experts.w3.weight") != weight.weight.end() &&
                           weight.weight.find(pre + ".ffn.shared_experts.w2.weight") != weight.weight.end()) {
                    Linear(ffnInput, weight[pre + ".ffn.shared_experts.w1.weight"], Data(), ww1);
                    Silu(ww1, ww1);
                    Linear(ffnInput, weight[pre + ".ffn.shared_experts.w3.weight"], Data(), ww3);
                    MulTo(ww1, ww3);
                    Linear(ww1, weight[pre + ".ffn.shared_experts.w2.weight"], Data(), sharedExpertOut);
                }
                DeepSeekV4ProfileLog("shared-expert-after", layer, startPos, seqlen, layerStartMs);
                ...
                DeepSeekV4ProfileLog("merge-moe-before", layer, startPos, seqlen, layerStartMs);
                MergeMOEBlock(&ffnInput, &expertIndex, &expertScore,
                              ...
                              MoeGateSwiglu, this->swiglu_limit);
                DeepSeekV4ProfileLog("merge-moe-after", layer, startPos, seqlen, layerStartMs);
            ...
            DeepSeekV4ProfileLog("moe-done", layer, startPos, seqlen, layerStartMs);
            ffnOut.Reshape(ffnDims);
            HcPostReference(ffnOut, hiddenStates, ffnMix, hiddenStates);
            DeepSeekV4ProfileLog("layer-done", layer, startPos, seqlen, layerStartMs);
        }
        ...
        DeepSeekV4ProfileLog("forward-done", -1, startPos, seqlen, forwardStartMs);
        if (profileDsv4) {
            auto *executor = GetProfilerExecutor();
            if (executor != nullptr) {
                executor->PrintProfiler();
            }
        }
        return ret;
    }

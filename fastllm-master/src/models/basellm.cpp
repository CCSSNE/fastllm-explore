...
namespace fastllm {
    static bool FastllmDebugResponseLoopEnabled() {
        const char *v = std::getenv("FASTLLM_DEBUG_RESPONSE_LOOP");
        if (v == nullptr) {
            return false;
        }
        std::string s(v);
    ...
    static void FastllmDebugResponseLoopLog(const std::string &message) {
        if (!FastllmDebugResponseLoopEnabled()) {
            return;
        }
        printf("[fastllm-response-loop] %s\n", message.c_str());
        fflush(stdout);
    }
...
// PrintProfiler();
    ...
    void basellm::NewMainLoop() {
        ...
        if (FastllmDebugResponseLoopEnabled()) {
            ...
            oss << "new engine limits maxTotalLens=" << maxTotalLens
                << " totalPages=" << totalPages
                << " pagesLimit=" << pagesLimit
                << " pageLen=" << pageLen
                << " maxBatch=" << maxBatch;
            FastllmDebugResponseLoopLog(oss.str());
        }
    ...
    int basellm::LaunchResponseTokens(const std::vector<int> &inputTokens,
        ...
        if (mainLoop == nullptr) {
            if (mainLoop == nullptr) {
                ...
                if (useNewEngine) {
                    FastllmDebugResponseLoopLog("start new engine");
                    mainLoop = new std::thread([](basellm *model) {
                        model->NewMainLoop();
                    }, this);
                } else {
                FastllmDebugResponseLoopLog("start old engine");
                mainLoop = new std::thread([](basellm *model) {
                    ...
                    if (FastllmDebugResponseLoopEnabled()) {
                        std::ostringstream oss;
                        oss << "old engine limits kvBytes=" << kvCacheLimit
                            << " maxTotalLens=" << maxTotalLens
...
*/
                                if (isPrompt && lenSum + it.second->currentTokens.size() + (currentActivate + 1) * 256 > maxTotalLens) {
                                    if (FastllmDebugResponseLoopEnabled()) {
                                        std::ostringstream oss;
                                        oss << "skip prefill handle=" << it.first
                                            << " reason=token-limit lenSum=" << lenSum
                                            << " curTokens=" << it.second->currentTokens.size()
                        ...
                        if (seqLens.size() > 0) {
                            if (FastllmDebugResponseLoopEnabled()) {
                                std::ostringstream oss;
                                oss << "old engine selected handles=" << handles.size()
                                    << " seqLen0=" << seqLens[0]
                                    << " ids=" << ids.size();
                                FastllmDebugResponseLoopLog(oss.str());
                            }
                                ...
                                if (seqLens[0] > first) {
                                    ...
                                    if (context->multimodalInput.size() > 0) {
                                        ret = model->ForwardMultimodal(inputIds,
                                                            attentionMasks[0] == nullptr ? Data() : *attentionMasks[0],
                                                            positionIds[0] == nullptr ? Data() : *positionIds[0], *pastKeyValue1, context->multimodalInput,
...
*/
        ...
        if (FastllmDebugResponseLoopEnabled()) {
            std::ostringstream oss;
            oss << "created handle=" << handleId
                << " inputTokens=" << inputTokens.size()
                << " outputLimit=" << generationConfig.output_token_limit
                << " addSpecial=" << generationConfig.add_special_tokens
                << " kvLimit=" << this->kvCacheLimit;
            FastllmDebugResponseLoopLog(oss.str());
        }
